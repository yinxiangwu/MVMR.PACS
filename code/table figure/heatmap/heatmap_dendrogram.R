...
enough_iv_strength <- 
  ...
  clustering_results <- t(sapply(res_dt$grouping_hist,function(x) {as.numeric(strsplit(x, "-")[[1]])}))
...
significance_df <- ifelse(res_dt$beta.hist == 0, 0, (res_dt$beta.hist)*(2*pnorm(abs(res_dt$beta.hist/res_dt$se.hist),lower.tail = FALSE) < 0.05))
...
# sig_matrix <- sig_matrix[res_dt$iv.hist.dt1 > 7, ]
...
# Plot the heatmap
jpeg('~/OneDrive - UW/UW biostats/2023-24 RA/MR with highly correlated exposure/manuscript/code/table figure/heatmap/heatmap_5e08.jpeg',width = 3200,height = 2400,res = 600)
Heatmap(sig_matrix,
        name = "Beta",
        col = col_fun,
